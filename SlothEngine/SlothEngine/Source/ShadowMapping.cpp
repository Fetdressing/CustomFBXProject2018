...
#include "..\Headers\ShadowMapping.h"
...
void ShadowMap::SetShadowMapConstants(int size, float depth, float Near, int matrixType, float orthosize){
	...
	if (matrixType == 1)
	{
		lightCam.SetLens(0.25f*3.14f, float(size / size), Near, depth);
...
void ShadowMap::DrawDepthMap(std::vector<MeshObject*> Mesh, ID3D11DeviceContext* deviceContext){
	...
	for (int i = 0; i < Mesh.size(); i++)
	{
		...
		deviceContext->Draw(Mesh[i]->nrTextureElements * 3, 0);
	}
